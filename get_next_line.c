...
/*   Updated: 2024/12/04 13:56:21 by aakyuz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_strchr(const char *s, int c)
{
	...
	i = 0;
	m = (unsigned char *)s;
	while (i != (ft_strlen_gnl((char *)m) + 1) && m)
	{
		if (s[i] == d)
...
static char	*set_line(char *line)
{
	...
	if (line[i] == 0 || line[1] == 0)
		return (NULL);
	left_c = ft_substr_gnl(line, i + 1, ft_strlen_gnl(line) - i);
	if (*left_c == 0)
	{
		free(left_c);
		left_c = NULL;
...
char	*get_next_line(int fd)
{
	...
	buffer = (char *)malloc(((unsigned long long)BUFFER_SIZE + 1)
			* sizeof(char));
	if (fd < 0 || (unsigned int)BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
	{
		free(left_c);
		free(buffer);
		left_c = NULL;
